...
void Surface::ImageLoad( char* a_File )
{
...
Pixel Surface::Get( int x, int y )
{ 
	if ((x >= 0) && (y >= 0) && (x < m_Width) && (y < m_Height)) return m_Buffer[x + y * m_Pitch];
	else return 0xBADFEED;
}
...
void Surface::AddBlendCopyTo( Surface* a_Dst, int a_X, int a_Y )
{
	Pixel* dst = a_Dst->GetBuffer();
	Pixel* src = m_Buffer;
	if ((src) && (dst)) 
...
void Surface::SubBlendCopyTo( Surface* a_Dst, int a_X, int a_Y )
{
	Pixel* dst = a_Dst->GetBuffer();
	Pixel* src = m_Buffer;
	if ((src) && (dst)) 
	{
		...
		if ((srcwidth > 0) && (srcheight > 0))
		{
			...
			for ( int y = 0; y < srcheight; y++ )
			{
				for ( int x = 0; x < srcwidth; x++ ) dst[x] = SubBlend( dst[x], src[x] );
				dst += dstpitch;
				src += srcpitch;
			}
		}
...
void Surface::ScaleRed( unsigned int a_Scale )
{
	int s = m_Pitch * m_Height;
	for ( int i = 0; i < s; i++ )
	{
		m_Buffer[i] = (((m_Buffer[i] & REDMASK) * a_Scale) >> 5) & REDMASK;
	}
}

void Surface::ScaleGreen( unsigned int a_Scale )
{
	int s = m_Pitch * m_Height;
	for ( int i = 0; i < s; i++ )
	{
		m_Buffer[i] = (((m_Buffer[i] & GREENMASK) * a_Scale) >> 5) & GREENMASK;
	}
}

void Surface::ScaleBlue( unsigned int a_Scale )
{
	int s = m_Pitch * m_Height;
	for ( int i = 0; i < s; i++ )
	{
		m_Buffer[i] = (((m_Buffer[i] & BLUEMASK) * a_Scale) >> 5) & BLUEMASK;
	}
}
		...
		for ( int y = 0; y < height; y++ )
		{
			...
			else if ( m_Flags & RFLARE )
			{
				...
				for ( int x = xs; x < width; x++ )
				{
					...
					{
						const Pixel c2 = *(dest + addr + x);
						*(dest + addr + x) = AddRed( c1, c2 );
					}
				}
			}
			else if ( m_Flags & GFLARE )
			{
				xs = (lsx > x1)?lsx - x1:0;
				for ( int x = xs; x < width; x++ )
				{
