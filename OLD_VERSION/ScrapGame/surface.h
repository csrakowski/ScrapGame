...
inline Pixel AddRed( Pixel a_Color1, Pixel a_Color2 )
{
	const unsigned int r = (a_Color1 & REDMASK) + (a_Color2 & REDMASK);
	const unsigned r1 = (r & REDMASK) | (REDMASK * (r >> 24));
	return ( r1 | ( a_Color2 & ( GREENMASK|BLUEMASK ) ) );
}

inline Pixel AddGreen( Pixel a_Color1, Pixel a_Color2 )
{
	const unsigned int g = (a_Color1 & GREENMASK) + (a_Color2 & GREENMASK);
	const unsigned g1 = (g & GREENMASK) | (GREENMASK * (g >> 16));
	return ( g1 | ( a_Color2 & ( REDMASK|BLUEMASK ) ) );
}

inline Pixel AddBlue( Pixel a_Color1, Pixel a_Color2 )
{
	const unsigned int b = (a_Color1 & BLUEMASK) + (a_Color2 & BLUEMASK);
	const unsigned b1 = (b & BLUEMASK) | (BLUEMASK * (b >> 8));
	return ( b1 | ( a_Color2 & ( GREENMASK|REDMASK ) ) );
}
...
public:
	...
	void ImageLoad( char* a_File );
	void CopyTo( Surface* a_Dst, int a_X = 0, int a_Y = 0 );
	void AddBlendCopyTo( Surface* a_Dst, int a_X, int a_Y );
	void SubBlendCopyTo( Surface* a_Dst, int a_X, int a_Y );
	void ScaleColor( unsigned int a_Scale );
	void ScaleRed( unsigned int a_Scale );
	void ScaleGreen( unsigned int a_Scale );
	void ScaleBlue( unsigned int a_Scale );
