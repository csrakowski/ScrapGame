...
{  
...
#ifdef FULLSCREEN
	SDL_SetVideoMode( SCRWIDTH, SCRHEIGHT, 0, SDL_OPENGL + SDL_FULLSCREEN );
#else
	SDL_SetVideoMode( SCRWIDTH, SCRHEIGHT, 0, SDL_OPENGL ); //+ SDL_NOFRAME
#endif
	...
	if (!init()) 
	{
		...
		vbo = false;
		SDL_WM_SetCaption( "Scrap Game - FALLBACK", NULL );
	}
	else SDL_WM_SetCaption( "Scrap Game", NULL );
	int exitapp = 0;
	game = new Game();
